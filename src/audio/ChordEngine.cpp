...
struct PadSound final : public juce::SynthesiserSound
{
    bool appliesToNote(int) override { return true; }
    bool appliesToChannel(int) override { return true; }
};

struct PadVoice final : public juce::SynthesiserVoice
{
    ...
    void startNote(int midiNoteNumber, float velocity, juce::SynthesiserSound*, int) override
    {
        ...
        angle_ = 0.0;
        level_ = velocity * 0.18;
        env_ = 0.0;
        releasing_ = false;
        attack_ = 1.0 - std::exp(-1.0 / (0.012 * getSampleRate())); // ~12 ms
        release_ = std::exp(-1.0 / (0.18 * getSampleRate()));       // ~180 ms
    }
    ...
    void renderNextBlock(juce::AudioBuffer<float>& output, int startSample, int numSamples) override
    {
        ...
        while (--numSamples >= 0)
        {
            ...
            const float sample = static_cast<float>((s + 0.18 * s2 + 0.07 * s3) * level_ * env_);
...
{
    synth_.addSound(new PadSound());
    for (int i = 0; i < 16; ++i)
        synth_.addVoice(new PadVoice());
